...
function teamNamesValidation(firstTeam, secondTeam) {
    if (firstTeam.toLowerCase() === secondTeam.toLowerCase()) {
        document.querySelector('#toss-coin #team-name-err').innerHTML = "Team name should be different from each other";
        return false;
    }
...
function tossCoin(event) {
    ...
    document.querySelector('#winner-name').innerHTML = "WHOOOREEE!! <b>" + teamNames[randomNum].toUpperCase() + "</b> HAS WON THE TOSS!!!"
    ...
    document.querySelector('#create-team').value = teamNames[randomNum].toUpperCase() + ", Select your team players.";
}
...
function buttonActiveHandler(playerType) {
    const playerSelectionButtons = document.querySelectorAll("#player-selecting-buttons .button-52");
...
function getSpecificPlayer(playerType) {
    ...
    if (isItSecondTeam) {
        filteredPlayerList = playerList.filter((player) => {
            return player.playingRole === playerType && player.teamMemberOf !== tossWinnerTeamName;
        });
    } else {
        filteredPlayerList = playerList.filter((player) => {
            ...
            `<div class="card" style = "width: 18rem;" >
                    <div class="card-body">
                      <div class="card-content">
                        ...
                        <i class="bi ${key.addedtoTeam ? "bi-check2-circle" : "bi-plus-circle"}" onclick="addingPlayer('${key.name}','${key.playingRole}','${key.credit}',${key.addedtoTeam})"></i>
                    </div>
                </div>`
    }
}
...
function loadDataforSelectingPlayer() {
    ...
    document.querySelector('.five h1').innerHTML = currentBattingTeamName + " Choose your players";
...
function removePlayer(teamList, playerDetails) {
    let updatedPlayerList = teamList.filter((player) => player.name !== playerDetails.name);
...
function addingPlayer(playerName, playingRole, credit, addedtoTeam) {
    ...
    const playerObj = new PlayersInfo(playerName, playingRole, parseInt(credit), currentBattingTeamName);
...
function creditValidation(playerDetails) {
    ...
    if (totalUpcomingCreditScores > 100) {
        document.querySelector('#err-credits').innerHTML = `This player exceed 100 credit score limit. Please select player whose credit score is less than ${101 - creditScores}`;
        return false
    }
    else {
...
function teamPlayerCounting(teamList) {
    ...
    const categoryWisePlayerCount = teamList.reduce((playerCounting, currentPlayer) => {
        if (playerCounting[currentPlayer.playingRole]) {
            playerCounting[currentPlayer.playingRole] = ++playerCounting[currentPlayer.playingRole];
        }
        else {
...
function displayTeamPlayerCountingWithCredits() {
    ...
    document.querySelector('#batsman-selected').innerHTML = Batsman ? Batsman : "";
    document.querySelector('#bowler-selected').innerHTML = Bowler ? Bowler : "";
    document.querySelector('#wk-selected').innerHTML = Wicketkeeper ? Wicketkeeper : "";

    let creditScores = countTeamCredits(teamList);

    document.querySelector('#credit').innerHTML = creditScores ? creditScores : "";
}
...
function finalTeamBuildingValidation() {
    ...
    if (!(Batsman === 5 && Bowler === 5 && Wicketkeeper === 1)) {
        document.querySelector('#err-teamCount').innerHTML = "Batsman/Bowler/Wicketkeeper should be select as 5/5/1";
        return false;
    }
    else {
    ...
    if (!(captain.length && viceCaptain.length)) {
        document.querySelector('#err-teamCount').innerHTML = "Captain/Vice captain should be selected from captain & viceCaptain section";
        return false;
    }
    else {
        document.querySelector('#err-teamCount').innerHTML = "";
...
function showingAllPlayerInTeams(teamList) {
    ...
    for (let key of teamList) {
        document.querySelector('#player-list').innerHTML +=
            `<div class="card" style = "width:80%; height:60px;" >
                    <div class="card-body">
                        <h5 class="player-name">${key.name}</h5>
                        <p class="playing-role">${key.playingRole}</p>
                        <p class="credit-score">Credit Score: ${key.credit}</p>
                        <button type="button" class="btn btn-primary" onclick="assignCaptaincy('${key.name}','captain')">${key.captain ? "Captain Selected" : "Select Captain"}</button>
                        <button type="button" class="btn btn-primary" onclick="assignCaptaincy('${key.name}','viceCaptain')">${key.viceCaptain ? "ViceCaptain Selected" : "Select ViceCaptain"}</button>
                    </div>
                </div>`
    }
}
...
function assignCaptaincy(name, captancyType) {
    ...
    if (captancyType === "captain") {
        teamList = teamList.map((player) => {
            ...
            else if (alreadyAssignedCaptain.length && player.name === alreadyAssignedCaptain[0].name) {
        ...
        teamList = teamList.map((player) => {
            ...
            else if (alreadyAssignedViceCaptain.length && player.name === alreadyAssignedViceCaptain[0].name) {
...
function summaryHandler() {
    ...
    if (teamList.length >= 2) {
        ...
        document.querySelector('#err-teamCount').innerHTML = "To Assign Captain team should have atleast two player";
    }
}
...
function saveTeam() {
    ...
    if (currentBattingTeamName === tossWinnerTeamName) {
        ...
        document.querySelector('h1').innerHTML = currentBattingTeamName + " Choose your players";
        displayTeamPlayerCountingWithCredits()
        getSpecificPlayer("Batsman");
    }
    else {
        ...
        document.querySelector("#rules #match-team-title").innerHTML = tossWinnerTeamName + " VS " + tossLoserTeamName;
...
function startMatch() {
    ...
    document.querySelector("#live-match #vs-heading").innerHTML = tossWinnerTeamName + " VS " + tossLoserTeamName;
...
function updateTeamScore() {
    ...
    switch (shotType) {
        case 1: batterPoints *= 1; break;
        case 2: batterPoints *= 2; break;
        case 3: batterPoints *= 3; break;
        case 4: batterPoints *= 5; break;
        case 6: batterPoints *= 8; break;
        default: break;
    }
    ...
    let battingTeam = currentBattingTeamName === tossWinnerTeamName ? tossWinnerTeamList : tossLoserTeamList;
    let bowlingTeam = currentBattingTeamName === tossWinnerTeamName ? tossLoserTeamList : tossWinnerTeamList;
    ...
    if (currentBattingTeamName === tossWinnerTeamName) {
        updateScoreBoardHandler(tossWinnerScoreBoard, tossLoserScoreBoard, shotType, batterPoints, bowlerPoints, currentBatsman.runs)
        ...
        updateScoreBoardHandler(tossLoserScoreBoard, tossWinnerScoreBoard, shotType, batterPoints, bowlerPoints, currentBatsman.runs);
    ...
    if (currentBowler.ballThrow === 6) {
        ...
        let over = +document.querySelector("#bowling-team-score #team-over #over").innerHTML;
...
function updateScoreBoardHandler(battingTeam, bowlingTeam, shotType, batterPoints, bowlerPoints, batsmanRuns) {
...
function inningCheckingHandler() {
    ...
    if (innigs === 1) {
        ...
        currentSessionCompleted("Match is Over press continue to see score-card", "score-card")
...
function showWinnerTeamName() {
    ...
    if (tossWinnerScoreBoard.points > tossLoserScoreBoard.points) {
        ...
        document.querySelector('#winner-team-name').innerHTML = tossWinnerTeamName + " won by " + pointDifference + " points";
    }
    else {
        pointDifference = tossLoserScoreBoard.points - tossWinnerScoreBoard.points;

        document.querySelector('#winner-team-name').innerHTML = tossLoserTeamName + " won by " + pointDifference + " points";
    }
}
...
function renderScoreCardTable(teamType) {
    ...
    if (teamType === 'winner') {
        ...
        statics = tossWinnerScoreBoard.runs + " - " + tossWinnerScoreBoard.wicket + " (" + tossWinnerScoreBoard.over.toFixed(2) + ") ";
        teamList = tossWinnerTeamList;
        points = tossWinnerScoreBoard.points;
    }
    else {
        teamName = tossLoserTeamName;
        statics = tossLoserScoreBoard.runs + " - " + tossLoserScoreBoard.wicket + " (" + tossLoserScoreBoard.over.toFixed(2) + ") ";
        teamList = tossLoserTeamList;
        points = tossLoserScoreBoard.points;
    }
    document.querySelector(`#${teamType}-score-card #toss-${teamType}-innig`).innerHTML = teamName;

    document.querySelector(`#${teamType}-score-card #run-wicket-over-statics`).innerHTML = statics;

    for (let player of teamList) {
        playerCount++;

        if (player.bowled) {
            document.querySelector(`#${teamType}-score-card #player-scores`).innerHTML += `
                <tr class="text-center" >
                        <th colspan="1">${playerCount}</th>
                        <th colspan="3">${player.name}  ${player.captain ? '(C)' : player.viceCaptain ? '(VC)' : ''}</th>
                        <th colspan="2">OUT</th>
    ...
    `
                ...
                document.querySelector(`#${teamType}-score-card #did-not-bat`).innerHTML += player.name + " , ";
            }
    ...
    document.querySelector(`#${teamType}-score-card #total-points`).innerHTML = points;
    ...
    for (let bowler of bowlers) {
        ...
        if (bowler.ballThrow) {
            console.log(document.querySelector(`#${teamType}-score-card #bowlers #bowler-scores`));
            document.querySelector(`#${teamType}-score-card #bowler #bowlers-scores`).innerHTML +=
                `
            <tr class="text-center" >
                <th colspan="1">${playerCount}</th>
            ...
            document.querySelector(`#${teamType}-score-card #bowler #did-not-bowl`).innerHTML +=
                bowler.name + "   "
        }
    }
...
function updateTeamName(battingTeamName, bowlingTeamName) {
    // Batting Section
    document.querySelector("#batting-team-score #name").innerHTML = battingTeamName + " - (BATTING)";

    // Bowling section
    document.querySelector("#bowling-team-score #name").innerHTML = bowlingTeamName + " - (BOWLING)";
...
function currentPlayerHandler() {
    ...
    let currentBowler = bowlingTeam.find((player) => player.playingRole === "Bowler" && !player.bowling);
...
function passingScoreHandler(playerChanged, willChagePlayer) {
    ...
    const { name, runs, fantasyPoints, fours, sixes, captain: batsmanCaptain, viceCaptain: batsmanViceCaptain } = currentBatsman;
    const { name: bName, wicketTaken, ballThrow, captain: bowlerCaptain, viceCaptain: bowlerViceCaptain } = currentBowler;
    ...
    displayScoreHandler(teamRuns, points, wicket, name, runs, fantasyPoints, fours, sixes, batsmanCaptain, batsmanViceCaptain, over, bName, wicketTaken, ballThrow, bowlingPoints, bowlerCaptain, bowlerViceCaptain);

}

function displayScoreHandler(teamRuns, teamPoints, teamWickets, batterName, batterRuns, batterPoints, batterFours, batterSixes, batterCaptain, batterViceCaptain, over, bowlerName, bowlerWickets, ballThrow, bowlingPoints, bowlerCaptain, bowlerViceCaptain) {
    ...
    document.querySelector("#batting-team-score #wickets").innerHTML = teamWickets;
    ...
    document.querySelector("#batter-name").innerHTML = batterName + batsmanPosition
    ...
    // team
    document.querySelector("#bowling-team-score #team-over #over").innerHTML = over.toFixed(1);
    document.querySelector("#bowling-team-score #team-points #points").innerHTML = bowlingPoints;
    ...
    document.querySelector("#bwl-wkt-stat").innerHTML = bowlerName + bowlerPosition + wicketTaken;
    ...
    ballThrow && (document.querySelector("#bwl-details").innerHTML += " Ball " + ballThrow + " - " + currentTimeStampWithShotType)
}
